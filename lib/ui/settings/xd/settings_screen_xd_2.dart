...
enum LocalData { hasData, noData, loading }

class SettingsScreenXD2 extends StatefulWidget {
  SettingsScreenXD2({
    Key key,
  }) : super(key: key);

  @override
  _SettingsScreenXD2State createState() => _SettingsScreenXD2State();
}

class _SettingsScreenXD2State extends State<SettingsScreenXD2> {
  bool isSwitchedAvgSpendingTime = false;
  bool isSwitchedUserReview = false;
  bool isSwitchedCrowding = false;
  bool isSwitchedAreaInUse = false;

  double _value = 0.0;

  var _onDataSaveLocally = PublishSubject<LocalData>();

  bool isDataSavedLocally;
  ...
  void initState() {
    super.initState();
    _onDataSaveLocally.add(LocalData.loading);
    getDataFromSharedPref();
  }

  void getDataFromSharedPref() async {
    isDataSavedLocally = await LocalDataSourceImpl().getDataSavedLocally();
    // data has been saved in local storage before
    if (isDataSavedLocally) {
      isSwitchedAvgSpendingTime =
          await LocalDataSourceImpl().getAvgSpendingTime();
      isSwitchedUserReview = await LocalDataSourceImpl().getUserReview();
      isSwitchedCrowding = await LocalDataSourceImpl().getCrowding();
      isSwitchedAreaInUse = await LocalDataSourceImpl().getAreaInUse();

      _value = await LocalDataSourceImpl().getRange();

      _onDataSaveLocally.add(LocalData.hasData);

      print(
          '<<<<<<<<<<<<<<<<<<<<<the isDataSavedLocally is $isDataSavedLocally>>>>>>>>>>>>>>>>>>>>>');
    }
    // data has not saved in local storage
    else {
      // default values
      isSwitchedAvgSpendingTime = true;
      LocalDataSourceImpl().setAvgSpendingTime(isSwitchedAvgSpendingTime);
      isSwitchedUserReview = true;
      LocalDataSourceImpl().setUserReview(isSwitchedUserReview);
      isSwitchedCrowding = true;
      LocalDataSourceImpl().setCrowding(isSwitchedCrowding);
      isSwitchedAreaInUse = false;
      LocalDataSourceImpl().setAreaInUse(isSwitchedAreaInUse);

      _value = 500.0;
      LocalDataSourceImpl().setRange(_value);

      _onDataSaveLocally.add(LocalData.noData);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        body: StreamBuilder<LocalData>(
            stream: _onDataSaveLocally,
            builder: (context, snapshot) {
              switch (snapshot.data) {
                case LocalData.hasData:
                        ...
                        Pinned.fromSize(
                          ...
                          child: Stack(
                            children: <Widget>[
                              Pinned.fromSize(
                                bounds:
                                    Rect.fromLTWH(-566.0, -49.0, 1446.0, 904.0),
                                ...
                                child:
                                  ...
                                  child: Stack(
                                    children: <Widget>[
                                      ...
                                      Pinned.fromSize(
                                        bounds:
                                            Rect.fromLTWH(7.0, 4.1, 9.0, 15.7),
                                        size: Size(24.0, 24.0),
                                        pinTop: true,
                                        pinBottom: true,
                                        fixedWidth: true,
                              ...
                              Pinned.fromSize(
                                ...
                                child:
                                  ...
                                  children: <Widget>[
                                    Pinned.fromSize(
                                      bounds:
                                          Rect.fromLTWH(0.0, 0.0, 47.0, 28.0),
                                      size: Size(48.0, 28.0),
                                      pinLeft: true,
                                      pinRight: true,
                                      pinTop: true,
                                    ...
                                    Pinned.fromSize(
                                      bounds:
                                          Rect.fromLTWH(0.0, 0.0, 48.0, 28.0),
                                      size: Size(48.0, 28.0),
                                      pinLeft: true,
                                      pinRight: true,
                                      pinTop: true,
